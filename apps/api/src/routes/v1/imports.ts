 ...
 */
import { importBatches, parents, parentStudent, students } from '@schoolmate/db';
  ...
  r.delete(
    ...
    async (request) => {
      const result = await request.tenantDb(async (db) => {
        ...
        const studentCount = countRows[0]?.n ?? 0;
        ...
        await writeAudit(db, request.auth!, {
          ...
          entityType: 'import_batch',
          entityId: batch.id,
          oldValues: { rowCount: batch.rowCount, tag: batch.tag },
        });
        return { rolledBack: true, studentsDeleted: studentCount };
      });
