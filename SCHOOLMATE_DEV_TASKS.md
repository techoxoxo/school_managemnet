...
| Phase 2       | Fees & examinations                        | 30    | 20   | 🟨      |
...
| P2-MOD-03 | Fee structure builder UI: heads, frequencies, installment plans, late-fee config                               | P2-MOD-02            | ✅     | `/fees/structures` page: branch/session/class selectors (client-filtered) + dynamic line-item rows (head/₹/frequency) → create → allocate to class. Verified live (created a structure, allocated 12 dues). BFF now omits content-type on bodyless POSTs (fixes /allocate).                                                                                                                      |
...
| P2-MOD-06 | Fee collection desk UI: search student → outstanding view → collect (cash/cheque/UPI ref) → receipt            | P2-MOD-04            | ✅     | `/fees` page: search → outstanding dues (₹) → collect (method/ref) → receipt PDF + payment history. Verified live in-browser (FIFO applied, ₹23k→₹18k, valid receipt PDF). BFF proxy now streams binary responses.                                                                                                                                                                               |
...
| P2-MOD-08 | Payment gateway integration (Razorpay first): order creation, webhook handler (idempotent), reconciliation job | P2-MOD-06            | 🟨     | `fee_payment_orders` table + RLS. `POST /v1/students/:id/payment-orders` (live via RAZORPAY_* keys, else local manual mode). Public `POST /webhooks/razorpay`: raw-body HMAC verify → withTenant settle via shared `applyPaymentFifo`, idempotent (created→paid atomic). Creds plumbed in .env (empty = manual). 5 offline tests green. Reconciliation job deferred (needs live creds + worker). |
...
| P2-MOD-15 | Marks entry UI: spreadsheet-style grid, teacher-scoped (own subjects only via ABAC), absent/exempt flags | P2-MOD-14, P0-AUTH-06 | ✅     | `/marks` page: pick exam → subject → spreadsheet grid with marks/absent, auto-grade on save. Verified live (87 → A2). Teacher ABAC enforced server-side (subject_teachers).                                                                                                                                                                                                                        |
...
| P2-MOD-20 | Bulk report generation: Bull queue worker, S3 caching, batch notify                                      | P2-MOD-18             | 🟨     | `POST /v1/exams/:id/report-cards/generate` renders all **published** cards → caches each PDF in S3 (`tenants/<t>/report-cards/<exam>/<student>.pdf`) → emits `exam.report_cards.generated` (batch notify) + audit. Shared `assembleReportCards` helper. Test fetches cached bytes back → %PDF. Synchronous for now — moving to the outbox worker needs PDF+storage in a shared package (deferred). |
| P2-MOD-21 | Result publishing: controlled release, pre-generation + cache warming before publish                     | P2-MOD-20             | ✅     | Plan §20 result-day spike. Controlled release tested (publish/unpublish; hidden until published; 409 if uncomputed). Pre-generation = bulk gen (P2-MOD-20) warms the S3 cache; the on-demand report-card PDF serves cached bytes (byte-equality tested) and recompute/unpublish invalidate it.                                                                                                     |
| P2-MOD-22 | Academic analytics: subject-wise averages, pass %, distribution charts                                   | P2-MOD-17             | ✅     |                                                                                                                                                                                                                                                                                                                                                                                                    |
| P2-QA-01  | E2E: fee collect (cash + gateway sandbox) + marks → publish → report card PDF                            | all above             | 🟨     | Phase exit gate. `e2e/phase2-exit-gate.spec.ts`: (A) fee structure→allocate→collect(cash)→receipt PDF; (B) marks→compute→publish→report-card PDF. Both green (PDF byte-asserted). Gateway-sandbox leg deferred with P2-MOD-08.                                                                                                                                                                     |
